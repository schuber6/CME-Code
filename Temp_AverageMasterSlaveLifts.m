...
for i=1:length(N)
    if N(i).class==0
        ...
        if length(used)>=7 && mean(N(i).Slave5x5(used))>=SlaveCutoff %~isempty(used) %Eliminate too short traces and traces in cells not expressing the slave flourophore
        ...
        for i2=1:length(used)
            ...
            AVtraceLifeactC{1+length(used)-i2}=[AVtraceLifeactC{1+length(used)-i2} N(i).SlaveBS(used(i2))];%N(i).Slave5x5(used(i2))/25-(N(i).Slave7x7(used(i2))-N(i).Slave5x5(used(i2)))/(49-25)];
