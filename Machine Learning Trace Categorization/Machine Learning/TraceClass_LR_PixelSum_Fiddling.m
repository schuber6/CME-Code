...
NBins=10;
[X,yBeg,yEnd,LN,AN]=ConstructFeatures(trainData,NBins,3);
[Xcv,yBegcv,yEndcv]=ConstructFeatures(CVData,NBins,3,LN,AN);
...
for i=1:length(NBins)
    [X,yBeg,yEnd,LN,AN]=ConstructFeatures(trainData,NBins(i),3);
    [Xcv,yBegcv,yEndcv]=ConstructFeatures(CVData,NBins(i),3,LN,AN);
    Theta=RegLogisticResgression(X,yEnd,lambda);
    ...
    [Err(ind,1),~]=FullErrorRate(preB,preE,[CVData.Decision]);
    Err(ind,2)=lambda;
    Err(ind,3)=NBins(i);
    ind=ind+1;
end
...
plot(NBins,ErrTe,'r--')
% Error around 21%
%save('190204_LRThetas_NBins10_B3.mat','ThetaB','ThetaE')
