function AverageMasterSlave_IntCohortsF_PB(N)
...
for i=1:length(N)
    if N(i).class==0
        used=find(N(i).int_PB>=IntCutoff,NF,'last');
        Coh=NCoh+1;
        
        for i2=1:NCoh
            if median(N(i).int_PB)<i2*ICoh
                Coh=i2;
                break
            end
        end
        if  mean(N(i).Slave5x5(used))>=SlaveCutoff && Coh~=0 && length(used)>=7%~isempty(used) %length(used)>=7 %Eliminate too short traces and traces in cells not expressing the slave flourophore
            OrdLif=sort(N(i).SlaveBS_PB,'descend');
            OrdAP2=sort(N(i).int_PB,'descend');
%             MaxLifeact=[MaxLifeact OrdLif(3)];
%             MaxAP2=[MaxAP2 OrdAP2(3)];
            MaxLifeact=[MaxLifeact median(N(i).SlaveBS_PB(used))];
            MaxAP2=[MaxAP2 median(N(i).int_PB(used))];
            Lifet=[Lifet length(N(i).frame)];
        medZ=median(N(i).st(used));
        %medZ=min(N(i).st(used));
        for i2=1:length(used)
            AVtraceINT(1+length(used)-i2)=AVtraceINT(1+length(used)-i2)+N(i).int_PB(used(i2))/max(N(i).int_PB);
            ...
            AVtraceINTC{Coh,1+length(used)-i2}=[AVtraceINTC{Coh,1+length(used)-i2} N(i).int_PB(used(i2))]; %/max(N(i).int_PB)];
            AVtraceZC{Coh,1+length(used)-i2}=[AVtraceZC{Coh,1+length(used)-i2} (N(i).st(used(i2))-medZ)*zg(i0)];
            AVtraceLifeactC{Coh,1+length(used)-i2}=[AVtraceLifeactC{Coh,1+length(used)-i2} N(i).SlaveBS_PB(used(i2))];%N(i).Slave5x5(used(i2))/25-(N(i).Slave7x7(used(i2))-N(i).Slave5x5(used(i2)))/(49-25)];
