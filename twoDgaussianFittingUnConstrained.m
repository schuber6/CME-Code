function c = twoDgaussianFittingUnConstrained(imgfile)
% c(1) = background
% c(2) = amplitude
% c(3) = x center
% c(4) = y center
...
%function to fit
F = @(back, amp, x0, y0, sx, sy, th, x, y)back + amp*exp( ...
    -(cos(th)^2/(2*sx^2)+sin(th)^2/(2*sy^2))*(x-x0).^2 ...
    +2*(-sin(2*th)/(4*sx^2) + sin(2*th)/(4*sy^2))*(x-x0).*(y-y0) ...
    -(sin(th)^2/(2*sx^2)+cos(th)^2/(2*sy^2))*(y-y0).^2);

%starting point, upper, and lower bounds of fitting.
c0 =  double([mean(min(img)), max(img(:))-mean(min(img)), ceil(size(img,2)/2), ceil(size(img,1)/2), 1,           1,           0 ]);
low = double([min(img(:)),    mean(img(:))-min(img(:)),   0,                   0,                   0,           0,           0]);
up =  double([mean(img(:)),   max(img(:))-min(img(:)),    size(img,2),         size(img,1),         size(img,2), size(img,1), pi/2]);
