...
%tmpds = dir(fullfile(omd,SS));
    ...
    for i=1:length(tmpdm)
        ...
        for i2=1:length(used)
            if NSTAms(used(i2)).ZGaussSD(1)>=1.25 && NSTAms(used(i2)).ZGaussSD(1)<=4 && NSTAms(used(i2)).ZGaussX0(1)<7
            ...
            ZG(i2)=NSTAms(used(i2)).ZGaussX0(1);
            end
        end
        FP=median(ZG);
        used2=find(ZG>=FP-1 & ZG<=FP+1);
        ZG(used2)=ZG(used2)>FP;
...
%         Zc=AvOfNearestN(xs,ys,Clath,5,10);
        ZG2=AvOfNearestN(xs(used2),ys(used2),ZG(used2),1,1);
        %[A,B]=size(Z);
        %Zz=[];
        %Zz=log(Za./Zc);
        Zz=ZG2;
