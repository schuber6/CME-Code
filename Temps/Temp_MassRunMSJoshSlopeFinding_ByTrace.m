...
for i=1:length(filespre)
    load(filespre{i})
    [JFm_ac{i},JRm_ac{i},JFs_ac{i},JRs_ac{i}]=CountJoshSlopes_MS_ByTrace(fxycMS);
end
...
for i=1:length(filespost)
    load(filespost{i})
    [JFm_ao{i},JRm_ao{i},JFs_ao{i},JRs_ao{i}]=CountJoshSlopes_MS_ByTrace(fxycMS);
end
...
for i=1:length(filespre)
    load(filespre{i})
    [JFm_cc{i},JRm_cc{i},JFs_cc{i},JRs_cc{i}]=CountJoshSlopes_MS_ByTrace(fxycMS);
end
...
for i=1:length(filespost)
    load(filespost{i})
    [JFm_co{i},JRm_co{i},JFs_co{i},JRs_co{i}]=CountJoshSlopes_MS_ByTrace(fxycMS);
end

save 180105_MSJoshSlopes_ByTrace_Mass.mat