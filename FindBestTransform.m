function [Offset1,Stretch1,bestSS,Worked]=FindBestTransform(Y1,Y2)
...
%differences--if Y1 does not cover Y2, does not fit)
...
for Off=-length(Y1):length(Y1)
    for iS=0:nS
        Stretch=2^(iS/nS);
        X1=(X1i-Off)*Stretch;
        if max(X1)>=max(X2) && min(X1)<=min(X2)
            Y1r2=interp1(X1,Y1,X2);
            SS=0;
            for i=1:length(X2)
                SS=SS+(Y1r2(i)-Y2(i))^2;
            end
