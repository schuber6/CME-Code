function [BINTslopes,BSlaveV,BMasterV,BRatio,LsI]=BinMSRatiosRange(fxyc_struct,Start,Fin)
...
for i=1:length(fxyc_struct)
    for i2=1:length(fxyc_struct(i).sl)
        ...
        if GR~=0
            ...
            F=fxyc_struct(i).frame(i2);
            C=fxyc_struct(i).class;
            if MV>=MasterThresh && SV>=SlaveThresh && MV/SV>=RatioThresh  && F>=Start && F<Fin %&& C==3
                if GR<E(1)
