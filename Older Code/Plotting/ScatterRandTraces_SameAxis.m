function [InterM,InterS,MSDifs,MM,MS,MD,C,Mast,Slave]=ScatterRandTraces_SameAxis(fxycMS)
...
while num<MN
    ...
    if LPrev>=ML && LPost>=ML && max(fxycMS(:,9,i))>MInt && max(fxycMS(:,9,i))>=0 && fxycMS(1,4,i)==3
        ...
        if C(1,2)>=MC
            ...
            t=(t-11)*3;
            if num==1
                ScatterWithArrows_Color(Mast,Slave,[1 0 0]);
                AddLinearRegLine_Color(Mast,Slave,[1 .7 0]);
            else
                ScatterWithArrows_Color(Mast,Slave,[0 0 1]);
                AddLinearRegLine_Color(Mast,Slave,[0 .7 1]);
            end
