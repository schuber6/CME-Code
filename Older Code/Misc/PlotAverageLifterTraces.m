function [LifterMed,SinkerMed,FlatMed]=PlotAverageLifterTraces(Lifters,Flat,Sinkers,NSTA,inds,NF,fg,zg)
...
LifterMed=zeros(1,length(Lifters));
SinkerMed=zeros(1,length(Sinkers));
FlatMed=zeros(1,length(Flat));


for i=1:length(Lifters)
    I=inds(Lifters(i));
    Fr=max(NSTA(I).frame);
    LifterMed(i)=median(NSTA(I).st);
%     if Fr>MinP && Fr<=MaxP 
