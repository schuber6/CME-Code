function [slopes,minint] = slope_finding_NoNorm(ints,frame_rate,bkgrd,varargin)
...
for i = 1:length(ints)
    ...
    for j = (rear+1):(lint-front)
        sub = (max(1,j-rear):min(lint,(j+front))); %indeces of current fitting
        %curmax = max(int)-bkgrd;
        curmax=1;  bkgrd=0; %This removes the normalization
        tmp = (int(sub)-bkgrd)/curmax; %Normalized fitting ints
        tmpx = sub*frame_rate;
        tmpy = tmp';
