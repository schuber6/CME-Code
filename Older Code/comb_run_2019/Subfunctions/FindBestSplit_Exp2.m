function [Split,varargout]=FindBestSplit_Exp2(Itrace,framegap,F1min,minL)
...
for i=2:length(Itrace)-2
    t{1}=Itrace(1:i);
    t{2}=Itrace(i+1:end);
    if length(t{1})>=minL && length(t{2})>=minL
        for i2=1:2
        ...
        used4=find(C==4);
        F3=F1(used);
        if (~isempty(used)) && isempty(used4) && (max(F1)>Forig || min(F1)>F1min(1) || max(F1)>F1min(2)) %If one of the traces is full, compare F1 score
