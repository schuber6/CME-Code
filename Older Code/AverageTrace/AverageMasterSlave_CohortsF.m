function AverageMasterSlave_CohortsF(N)
...
for i=1:length(N)
    if N(i).class==0
        used=find(N(i).int_PB>=IntCutoff);
        Coh=0;
        for i2=1:NCoh
        ...
        for i2=1:length(used)
            ...
            AVtraceINTC{Coh,1+length(used)-i2}=[AVtraceINTC{Coh,1+length(used)-i2} N(i).int_PB(used(i2))]; %/max(N(i).int)];
