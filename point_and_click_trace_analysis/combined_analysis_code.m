...
mod = [0,0,0];
ml = length(imfinfo(reconmovnm));    %length of the movie
ff = 1; lf = ml; df=1;
ind = 0;
ss = size(imread(reconmovnm));      %xy size of the movie
rxpos = NaN; rypos = NaN;
oxpos = NaN; oypos = NaN;
...
rimg = zeros([ss ml],'uint16');      %These store the movie in memory
...
minoc = min(oimg(:)); maxoc = max(oimg(:));   %These two set the intensity scale--can be circumvented if movie is large
...
graphx = (1-imgx)/2;
cfr = 1;
cintrad = 8;        %radius (in pixels) to look for spots in
ozrad = 5;
zrad = ozrad;
...
upz = false;
disp_er = true;
while true        %This loop is the main body of the code
    try
        waitfor(fh_img,'SelectionType','normal');
        [area, int, SNR] = deal(zeros(1,ml));
        zp = fh_img.CurrentPoint;   %Position clicked on
        tx = zp(1)*ss(2)+.5;
        ty = ss(1)-zp(2)*ss(1)+.5;
        ...
        if ind > 0        %True if the spot being checked is already saved
            mask = zeros([size(tracest(ind).mask(:,:,1)) ml]);
            mask(:,:,tracest(ind).frame) = tracest(ind).mask;
            tcfr = cfr;                    %cfr is the current frame
            fff = tracest(ind).frame(1);   %fff is the first frame of the trace
            flf = tracest(ind).frame(end); %flf is the last frame of the trace
            
            fdf = tracest(ind).donutframe; %fdf is the first donut frame
            if isempty(fdf)
                fdf=1;
            end
            ...
            cfr = fff;        %This chunk sets the first, last, and donut frame bars
            sff;
            cfr = flf;
            slf;
            cfr = fdf;
    ...
    function zoom_in
        ...
        if rxpos>zrad && rxpos<=ss(2)-zrad && rypos>zrad && rypos<=ss(1)-zrad
            if ind>0
                findex=find(tracest(ind).frame==cfr);
                if ~isempty(findex)
                    rxpos=tracest(ind).xpos(findex(1));
                    rypos=tracest(ind).ypos(findex(1));
                end
            end
            axes(ah_zoom_img)
    ...
    function overlay_mask  %Displays area mask on zoomed-in image
        persistent msh
        axes(ah_zoom_img)
        delete(msh)
        hold on
    ...
    function [cx, cy] = cofint(img,tmpx,tmpy,frame)
        ...
        tmp_mask = edge(tmpimg,'canny',[.5 .9])>0;  %Finds structure edges
    ...
    function [integ, SNR] = twoDgaussianFitting_theta(img, disp)  %Fits window to 2d gaussian
        % c(1) = background
        % c(2) = amplitude
        % c(3) = x center
        % c(4) = y center
    ...
    function scatter_points(frame)  %Displays all found spots over image
    ...
    function cf_ball   %Displays current frame as a red 'o' in the area plot
        persistent abh ibh
        axes(ah_area_graph)
    ...
    function frame_line(src,loc,col)  %Displays current frame as a line on the slider
        axes(src)
    ...
    function ind = already_found(xp,yp,frame)  %Checks if there is a spot close to xp yp (<cintrad=8 px distance away)
    ...
    function sff(~,~)  %sets first and last frames
        ff = cfr;
    ...
    function sdf(~,~)  %sets donut frame
        df = cfr;
        frame_line(ah_scroll,cfr,[.7 .7 0])
    end
