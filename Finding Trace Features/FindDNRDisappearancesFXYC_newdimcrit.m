function events=FindDNRDisappearancesFXYC_newdimcrit(fxyc)
%Finds points in the DNR intensity trace where DNR goes from significantly
%above the threshold to below in a few frames (<=6)
%Also intensity must be in above category for >1 frame, for the below
%category, there must be a frame directly before the dim frame where
%intensity is between bright and dim
Thresh=500;
BrightThresh=1000;
events=[];
    ...
    for i2=1:L
        if fxyc(i2,5,i)>=BrightThresh && i2+1<=L && fxyc(i2+1,5,i)>=BrightThresh
            for i3=1:min(6,L-(i2+2))
                if fxyc(i2+1+i3,5,i)<=min(fxyc(i2,5,i),fxyc(i2+1,5,i)) && fxyc(i2+2+i3,5,i)<=Thresh
