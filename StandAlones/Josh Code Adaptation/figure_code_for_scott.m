...
while true
    ...
    function zoom_in
        ...
        if ind==0
            ...
            rxpos = tracest(ind).xpos(tracest(ind).frame==cfr);
            rypos = tracest(ind).ypos(tracest(ind).frame==cfr);
        end
        if rxpos>zrad && rxpos<=ss(2)-zrad && rypos>zrad && rypos<=ss(1)-zrad
            axes(ah_zoom_img)
    ...
    function uind = already_found(xp,yp,frame)
        persistent uih_found
        uind = 0;
        if ind==0
            vec = 1:length(tracest);
        else
            vec = [ind:length(tracest) 1:ind-1];
        end
        delete(uih_found)
        for i = vec
