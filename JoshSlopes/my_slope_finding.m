function [slopes] = my_slope_finding(ints,frame_rate,bkgrd,varargin)
...
for i = 1:length(ints)
    ...
    for j = (rear+1):(lint-front)
        ...
        R = corrcoef(tmpy,intdif(j)*tmpx + numer2/denom);
        r2coef(j) = R(1,2);
    end
    slopes(i).sl = intdif;
    slopes(i).r2 = r2coef;%add data to existing structure.
